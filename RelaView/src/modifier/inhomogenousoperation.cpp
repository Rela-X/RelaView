...
//#---------------------------------------------------------------------------#
InhomogenousOperation::InhomogenousOperation(RF_RELATION* (*operation)(RF_DOMAIN*, RF_DOMAIN*, RF_ERROR*),
                                             const QString &name, const QString &buttonText,
                                             QObject *parent) :
    AbstractOperation(name, buttonText, parent)
{
    this->m_operation = operation;
    this->m_type = AbstractOperation::newInhomogenous;
}

//#---------------------------------------------------------------------------#
//#------- PUBLIC ------------------------------------------------------------#
//#---------------------------------------------------------------------------#
int InhomogenousOperation::executeOperation(RelationData* relation,
                                            const DomainData* d1,
                                            const DomainData* d2) const
{
    RF_ERROR error;
    RF_RELATION* newRelation =  (*this->m_operation)(d1->relaFixDomain(),
                                                    d2->relaFixDomain(),
                                                    &error);
